<nav class="sidebar-menu">
    <div class="navbar-brand mb-0 px-6">
        <div class="d-flex">
            <a class="brand" href="{{ url('/') }}">
                <div class="brand-title">{{ config('app.name') }}</div>
            </a>
        </div>
    </div>
    <ul class="nav nav-pills flex-column mb-auto">
        <li class="nav-item">
            <a href="#" class="nav-link">
                <span class="nav-title">Link</span>
            </a>
        </li>
        <li class="nav-item dropdown">
            <a href="{{ url('/') }}" class="nav-link dropdown-toggle" role="button" data-bs-toggle="collapse" data-bs-target="#collapseExample" aria-expanded="false" aria-controls="collapseExample">
                <span class="nav-title">Link</span>
            </a>
            <ul class="collapse" id="collapseExample">
                <li><a class="nav-link" href="#"><span class="nav-title">Action</span></a></li>
                <li><a class="nav-link" href="#"><span class="nav-title">Another action</span></a></li>
                <li><a class="nav-link" href="#"><span class="nav-title">Something else here</span></a></li>
            </ul>
        </li>
    </ul>
    <div class="sidebar-footer">

    </div>
</nav>
